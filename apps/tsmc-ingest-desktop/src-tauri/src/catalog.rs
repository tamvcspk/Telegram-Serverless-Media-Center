//! Hai Tauri command cho "Trình quản lý catalog" (A.4, ADR-0017 § addendum
//! "Trình quản lý catalog") — `check_deleted_messages`/`delete_message`, hai
//! ngoại lệ `IngestRpc` không có tương ứng 1-1 phía TS (xem doc comment
//! `ingest-rpc-trait/src/lib.rs`). Cả hai luôn thao tác trên
//! `state.selected_channel` (không nhận `ResolvedChannel` qua IPC), cùng quy
//! ước với `check_write_permission`/`read_pinned_catalog` ở `commands.rs`.
//!
...
use crate::dto::IngestRpcErrorDto;
