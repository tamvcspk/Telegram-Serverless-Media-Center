...
**Trạng thái: đang xây dần từng slice.** 16 ADR. Auth, Sync & Hydration, Index, Browse, Collections, Sources, Player, Ingest Editor, Settings đều chạy thật trên https://tsmc-staging.web.app (redesign kiểu Netflix cho Browse/Collections/Sources từ 2026-08-29). Lịch sử triển khai theo từng slice/ngày: [docs/changelog.md](docs/changelog.md). Việc còn lại theo từng màn hình/hạ tầng: [docs/roadmap.md](docs/roadmap.md). Bài học kỹ thuật đã trả giá bằng debug thật: [docs/lessons.md](docs/lessons.md). Quyết định kiến trúc ràng buộc + trạng thái kiểm chứng rủi ro hiện tại: [docs/architecture.md](docs/architecture.md) + [docs/adr/](docs/adr/). Spec UX từng màn hình (mockup, đối chiếu spec-vs-thật tại chỗ): [docs/ux-design.md](docs/ux-design.md).
...
- `docs/roadmap.md`, `docs/changelog.md`, `docs/lessons.md` không có skill riêng — cập nhật thủ công theo slice (xem "Cách cập nhật" ở đầu mỗi file).
