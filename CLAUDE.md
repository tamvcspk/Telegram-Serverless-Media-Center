...
**Trạng thái: đang xây dần từng slice.** 16 ADR. Auth (F1.1), Sync & Hydration (F1.2/F1.3), và Index (F2, ADR-0010) chạy thật, deploy tại https://tsmc-staging.web.app — dò/tạo kênh state, ghi event, đẩy lên Telegram, và 3 tầng index (catalog/delta/full-scan) + mô hình tin cậy phân tầng đã kiểm chứng bằng tài khoản thật, nhiều loại kênh (riêng, cộng đồng bị `CHAT_ADMIN_REQUIRED`, cộng đồng có publisher không phải admin). Nén snapshot (>200 event hoặc snapshot >7 ngày) và gộp nhiều kênh state (ADR-0014) chưa qua kiểm chứng thiết bị thật (không tự phát sinh trong một lần dùng bình thường). Browse/player chưa có — màn Index hiện tại là công cụ debug tối thiểu (thêm nguồn/quét/xem trust từng item), không phải UI cuối. 3 spike đã đóng bằng số liệu thật rồi xoá mã nguồn (số liệu giữ ở [docs/spikes/](docs/spikes/)); SPIKE-04/05 chưa dựng.
