...
- **`tsmc-ingest` CLI (ADR-0013 mục 1) — code xong lần đầu (2026-08-29), chưa chạy trên tài khoản/ffmpeg thật.** `apps/tsmc-ingest` + `libs/core-ingest` (phân hạng A/B/C/D, kế thừa metadata theo series, gộp catalog) + `libs/core-mtproto/src/gateway-ingest.ts` (`uploadVideoDocument()`). Pipeline: probe (`ffprobe` cục bộ) → phân hạng → remux/re-encode nếu cần → thumbnail → metadata (kế thừa từ tập trước cùng series, có từ v1 theo yêu cầu ADR) → upload → gộp + publish `catalog.json` một lần/batch. Chi tiết phát hiện lúc code (tách `SessionStoragePort` khỏi Dexie, esbuild bundle cho Node...): [ADR-0013 § Cập nhật 2026-08-29, lần code đầu tiên](./adr/0013-bot-dong-hanh-va-pipeline-ingest.md#cập-nhật-sau-khi-accepted-2026-08-29-tsmc-ingest-cli--lần-code-đầu-tiên). Còn thiếu: verify `login`/`probe`/`upload` bằng tài khoản Telegram thật + `ffmpeg` cài thật + kênh test thật (checklist ở [docs/pending-device-tests.md](./pending-device-tests.md)); đối chiếu ngưỡng phân hạng A/B/C/D với file mẫu thật (hiện chỉ test bằng fixture JSON); rút phụ đề mới lưu cục bộ, chưa upload kèm. `@tsmc_bot` (mục 2) và quyết định GUI Tauri vẫn để ngỏ, ngoài phạm vi CLI này. `[Cần kiểm chứng thiết bị thật]`
