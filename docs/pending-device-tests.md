...
- [ ] Kéo NGUYÊN MỘT FOLDER chứa nhiều tập phim (season thật) thả vào → `list_media_files` quét đệ quy, tất cả file video bên trong (đúng phần mở rộng: mp4/mkv/avi/mov/webm/ts/m4v/wmv/flv/mpg/mpeg/m2ts) xuất hiện trong hàng đợi, file không phải video (`.nfo`, `.srt`, ảnh poster...) bị bỏ qua đúng như thiết kế.
- [ ] Thả file KHÔNG PHẢI media (vd đổi tên `.txt` thành `.mp4`) → `probe_media` lỗi, item đó hiện "Lỗi" (không phải hạng màu), các item khác trong cùng lượt thả VẪN ra kết quả bình thường (không bị một file hỏng chặn cả batch — mockup A.5 "Pipeline FFmpeg crash: file đó đánh dấu Lỗi, batch chạy tiếp").
- [ ] Thả lại ĐÚNG file đã có sẵn trong hàng đợi → không bị thêm trùng (dedupe theo đường dẫn tuyệt đối).
- [ ] Bấm nút "×" trên một dòng → item biến mất khỏi hàng đợi, không ảnh hưởng các dòng khác.
...
- [ ] File RẤT LỚN (nhiều GB, HEVC 4K...) → `probe_media` không đơ UI lâu bất thường (đây chỉ đọc header/metadata, không decode toàn bộ frame — nên phải nhanh bất kể dung lượng file; nếu chậm thật, nghi ngờ đầu tiên là container có index đặt cuối file buộc phải seek xa).
...
- [ ] Sửa tay Title ở MỘT dòng bất kỳ → chỉ dòng đó đổi, các dòng khác giữ nguyên (kế thừa chỉ áp dụng lúc THẢ, không tự động lan truyền ngược/xuôi sau khi đã vào bảng).
- [ ] Gõ Season/Ep vào một dòng đang trống (item không khớp `SxxExx` lúc parse tên file, vd file "Extra.mp4") → ô nhận giá trị, hạng/tên file không đổi, cột 🚦 vẫn đúng như trước.
...
- [ ] Lúc mới chuyển sang "Đang upload video…" (trước khi có % byte đầu tiên) → avatar vẫn hiện spinner XOAY VÔ ĐỊNH (không phải một vòng trống/vô hình) — sau khi % đầu tiên tới, spinner chuyển sang xác định (có số ở giữa, tăng dần). **Open issue đã biết (2026-09-12, xem "Nếu có gì vỡ"): với Hạng B/C/D (mọi file phải qua remux/re-encode), spinner KHÔNG BAO GIỜ chuyển sang xác định — luôn vô định suốt cả quá trình upload, do lệch correlation id `path` giữa backend/frontend. CHƯA vá.**
...
- **Open issue đã biết, CHƯA vá (2026-09-12): progress upload luôn `indeterminate`, không bao giờ hiện % thật với Hạng B/C/D.** Nguyên nhân xác định qua đọc code (chưa vá): `upload.rs::upload_video()` bắn sự kiện `"upload-progress"` với `path: file_path` = đường dẫn file TẠM (`prepared.remuxed_path`, sinh trong `temp_dir` bởi `prepareUpload()`), nhưng `workspace.ts::onUploadProgress()` tìm item khớp theo `item.path` = đường dẫn file GỐC trên đĩa — hai giá trị KHÁC NHAU nên `updateQueueItem()` không tìm thấy phần tử nào để cập nhật, patch rơi im lặng, `item.progress` không bao giờ được set. Hạng A (không qua remux) CÓ THỂ tình cờ không gặp lỗi này nếu đường dẫn tạm trùng đường dẫn gốc — CHƯA xác nhận, cần test riêng. Xem chi tiết + hướng vá dự kiến ở [docs/changelog.md § 2026-09-12, wrap phiên — open issue progress indeterminate](./changelog.md#2026-09-12--gui-ingest-desktop-wrap-phiên--phát-hiện-chưa-vá-open-issue-progress-upload-luôn-indeterminate).
