...
- [x] **"invalid argument" TÁI DIỄN LẦN 3, 2026-09-17 — lần này ở VIDEO, không phải audio, chỉ xảy ra ở `reencode_to_mp4()` (Hạng D) — verify 2026-09-17, ĐẠT (user xác nhận "đã work" qua `cargo tauri dev` + tài khoản thật):** user báo lỗi trên `The Big Bang Theory S01E15.avi` (Xvid/mp3, 640×368, ~21 phút — cùng series với 2 lần trước nhưng KHÁC file, KHÔNG phải file đã vá ngày 2026-09-12/2026-09-15). Tái hiện bằng ví dụ chạy tay (`ingest-ffmpeg/examples/debug_reencode.rs`, đã xoá sau khi debug xong): file dùng "packed B-frames" kiểu Xvid/DivX cũ (ffmpeg tự cảnh báo "Video uses a non-standard and wasteful way to store B-frames") — MỘT packet đóng gói CẢ P-frame lẫn B-frame trễ, decoder tách ra 2 `receive_frame()` từ 1 `send_packet()` nhưng frame tách ra kế thừa TRÙNG pts với frame kế tiếp → sau khi encode lại H.264, hai packet output liên tiếp trùng dts → mp4 muxer chặn cứng ("Application provided invalid, non monotonically increasing dts to muxer in stream 0: 33784 >= 33784" = `AVERROR(EINVAL)`). Khác hẳn 2 lần trước (đều ở audio) — đây là VIDEO, và audio (mp3, không đổi channel layout) hoàn toàn bình thường. **Đã vá:** `reencode.rs::drain_video_encoder` theo dõi dts cuối đã mux (`last_video_dts`), bơm dts packet mới lên tối thiểu `last_dts + 1` (giữ bất biến dts≤pts) nếu phát hiện không tăng nghiêm ngặt sau `rescale_ts` — đúng cách `ffmpeg` CLI chính thức tự làm (`ffmpeg.c::bump_dts`) thay vì để muxer trả lỗi cứng như native API. Verify tay ngoài Tauri trước đó: build lại đúng file gây lỗi → không còn lỗi, `ffprobe` đọc output OK (30671 frame video khớp gốc), `ffmpeg -c copy` remux lại output (kiểm tra dts embedded qua muxer strict) không lỗi. Sau đó verify qua `cargo tauri dev` + tài khoản thật trên đúng file `S01E15.avi` — user xác nhận "đã work". Chưa có chi tiết riêng việc xem lại bằng mắt hình ảnh sau re-encode (không chặn, để mở nếu cần đối chiếu sau).
