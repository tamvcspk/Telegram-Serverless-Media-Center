...
> **Mã nguồn đã xoá (2026-09-13)** cùng lúc khai tử `apps/tsmc-ingest` CLI (ADR-0013 § addendum tương ứng) — logic FFmpeg native đã được port nguyên vẹn sang `apps/tsmc-ingest-desktop/ingest-ffmpeg` (production thật, verify tiếp bằng tài khoản Telegram thật) từ lâu, `tools/spike-09/` chỉ còn là bản sao lịch sử không còn cần giữ. Lịch sử mã nguồn vẫn còn trong git log nếu cần soi lại. Số liệu/phát hiện dưới đây (72.1x realtime, 3 bug thật, ranh giới crash) vẫn đứng vững — đây là kết quả ĐO ĐƯỢC, không phụ thuộc việc code còn nằm ở đâu.

### Bàn thử nghiệm

Crate Rust độc lập ở `tools/spike-09/` (đã xoá, xem ghi chú trên) — Cargo project, **không** nối vào pnpm workspace hiện có của repo — cùng nguyên tắc cô lập của [SPIKE-01](#spike-01)/[SPIKE-08](#spike-08): nếu spike hỏng, biết chắc lỗi nằm ở hành vi Rust/FFmpeg-FFI, không phải ở pipeline `tsmc-ingest` thật (pipeline đó đã verify riêng, xem ADR-0013).
...
**Vẫn CHƯA test lúc đó:** đường native cho chính bước remux nặng nhất (Hạng A/B/C) qua CLI thật — lần chạy 2026-09-04 dùng file Hạng D nên chỉ native-path cho `generateThumbnail()` được thực thi qua production, chưa phải `remuxToMp4`. Câu hỏi này đã MOOT sau khi `apps/tsmc-ingest` CLI bị khai tử (2026-09-13, xem ADR-0013 § addendum) — logic remux native đã verify đầy đủ qua GUI Tauri thật (`ingest-ffmpeg`) rồi, không còn CLI nào để tự bổ sung bằng chứng này nữa.
...
**Cập nhật 2026-09-13:** `tools/spike-09/` giữ nguyên đúng như dự kiến ("có giá trị làm điểm khởi đầu thật cho core lib Tauri") — và hướng đó ĐÃ được chọn (ADR-0017), code đã port hoàn chỉnh thành `apps/tsmc-ingest-desktop/ingest-ffmpeg` production thật, verify tiếp bằng tài khoản Telegram thật. Bản sao spike gốc không còn giá trị riêng nữa nên đã xoá (xem ghi chú đầu mục "Mã nguồn đã xoá" ở trên) — mọi cải tiến/bugfix sau này (vd `scaling::Context` ép YUV420P, `reencode_to_mp4()`) chỉ còn nằm ở `ingest-ffmpeg`, không đồng bộ ngược lại spike.
...
> **Mã nguồn đã xoá (2026-09-13)** cùng lúc khai tử `apps/tsmc-ingest` CLI (ADR-0013 § addendum tương ứng) — R3 (`grammers-client`) đã được chọn (ADR-0017) và port nguyên vẹn sang `apps/tsmc-ingest-desktop/ingest-grammers` production thật, `tools/spike-10/` chỉ còn là bản sao lịch sử. Lịch sử mã nguồn vẫn còn trong git log nếu cần soi lại. Mọi số liệu/phát hiện dưới đây (M1-M8, P1, hai bug môi trường, Đ1/Đ2) vẫn đứng vững nguyên vẹn — đây là kết quả ĐO ĐƯỢC, không phụ thuộc việc code còn nằm ở đâu.

### Trạng thái dựng (2026-09-05) — code thật, chưa phải số đo M1-M8

`tools/spike-10/` (đã xoá, xem ghi chú trên) từng có: workspace Cargo (`rpc-trait` + `r3-grammers` + `r4-ferogram`), CLI `login`/`upload` đối xứng cho cả hai nhánh Rust, `shared/generate-sample.mjs` (đã smoke-test clip 2 giây, chưa sinh bản đầy đủ ≥2 GB). `cargo build --workspace` sạch, không warning. `r1-webview`/`r2-sidecar` chưa có một dòng code nào.

**Phát hiện thật khi dựng** (đọc trực tiếp mã nguồn `grammers-client-0.10.0`/`ferogram-0.6.5` tải về `~/.cargo/registry/src/...` — không đoán từ README/docs.rs, cùng phương pháp `tools/spike-09/`; chi tiết đầy đủ từng nằm ở `tools/spike-10/README.md`, đã xoá — tóm tắt số liệu vẫn đầy đủ ở các mục dưới đây):
...
**2026-09-05, tài khoản thật, kênh `tsmc_mediacenter`.** Hai bug thật phát hiện lúc chạy: clap `--session` không nhận global, `.env` sai một cấp thư mục, `tl::enums::ChatFull` hai thư viện đặt tên biến thể khác nhau cho channel (chi tiết đầy đủ từng nằm ở `tools/spike-10/README.md`, đã xoá cùng lúc khai tử CLI — xem ghi chú đầu mục). Tóm tắt:
...
**Còn lại cho R3:** chỉ M6 (`FLOOD_WAIT`) — để ngỏ có chủ đích, không chủ động ép (CLAUDE.md + SPIKE-04 đã là nơi dò ngưỡng). R3 đạt mọi tiêu chí đo được khác (M1/M2/M3/M5/M7/M8/P1); M4 cải thiện đáng kể (13.5%→24.4%, không phải trần kiến trúc) nhưng chưa qua ngưỡng pass 80%. Đ1/Đ2 (đóng gói/giấy phép) sau này đo thật khi đóng gói `.exe` (2026-09-13) — cả hai ĐẠT, xem "Cập nhật sau khi đóng" ở cuối mục này; Đ3 (chi phí viết mới) vẫn chưa đo.

**M2 cho R4 giờ là KẾT LUẬN DỨT KHOÁT, không phải "chưa tìm ra cách":** thử vá bằng cách tự viết chunk-upload gọi thẳng `client.invoke()` (bypass `UploadedFile`) để vừa lấy tốc độ vừa tự gắn `DocumentAttributeVideo` — chạy thật ném `ConnectionReset` ở 1.7%. Đọc mã nguồn xác nhận: ferogram cố tình tách một "transfer pool" hoàn toàn riêng (auth key/transport/session riêng) cho file traffic, và hàm route vào đó (`rpc_transfer_on_dc_pub`) không phải API công khai — nghĩa là **không có cách an toàn nào từ ngoài crate vừa dùng đúng transfer pool vừa tự chọn attributes**.
