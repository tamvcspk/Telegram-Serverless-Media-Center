...
Phần lớn rủi ro dưới đây đã có bằng chứng thực nghiệm hoặc quyết định rõ ràng; dòng cuối bảng ([SPIKE-08](./spikes/README.md#spike-08)) vẫn là giả thuyết chưa đo, ưu tiên thấp. Bàn thử nghiệm nằm ở [docs/spikes/](./spikes/), chạy trên môi trường ở [ADR-0015](./adr/0015-moi-truong-kiem-thu-firebase-hosting.md). Dòng bị gạch ngang là rủi ro đã đóng (đã gỡ hoặc đã chấp nhận có chủ đích); dòng còn lại là rủi ro thường trực được giảm thiểu bằng thiết kế, không phải thứ một spike đơn lẻ có thể "xong".
...
| ~~API trình duyệt dùng để tự động hoá bước "probe" compat lúc ingest ([ADR-0013](./adr/0013-bot-dong-hanh-va-pipeline-ingest.md) mục 3, qua `scanSource()`) có thể trả lời sai~~ | ~~Tự động hoá bằng API sai sẽ báo compat sai có hệ thống~~ | 🟡 **Né tránh bằng quyết định kiến trúc (2026-08-29)**, không phải bằng spike — xem [ADR-0013 § Cập nhật "phản biện auto-probe"](./adr/0013-bot-dong-hanh-va-pipeline-ingest.md#cập-nhật-sau-khi-accepted-2026-08-29-phản-biện-auto-probe-sau-khi-quét--quyết-định-thứ-tự-xây): probe qua mạng lúc quét vừa tốn ≥2 round-trip/file (`moov` cuối file) vừa bắt tài khoản người xem gánh rủi ro `FLOOD_WAIT` diện rộng (cùng loại rủi ro đã cấm ở ADR-0010 §3) — quay lại `ffprobe` cục bộ trên máy admin (`tsmc-ingest` CLI, chưa code) thay vì browser API, xoá hẳn rủi ro này khỏi đường ingest |
| API trình duyệt nào khớp đúng khả năng phát thật của `<video src>` — nếu sau này `Player.checkCompat()` muốn cảnh báo cá nhân hoá theo từng trình duyệt/thiết bị đang xem (thay vì chỉ tin nhãn `compat` tĩnh trong catalog) | Cảnh báo sai theo thiết bị (vd Safari giải mã HEVC được nhưng vẫn bị cảnh báo `partial` như Chrome) — ảnh hưởng nhỏ, chỉ là UI cảnh báo phụ, không chặn phát | ⏳ **Chưa kiểm chứng, ưu tiên thấp** — [SPIKE-08](./spikes/README.md#spike-08) mở, chưa dựng tool, không gate việc nào đang làm |
| ~~GUI ingest desktop (Tauri) chạy trong môi trường **không có Node**, trong khi đường upload đã verify thật của [ADR-0013](./adr/0013-bot-dong-hanh-va-pipeline-ingest.md) dựa vào `sendFile(path)` của GramJS — vốn stream từ đĩa bằng `fs` của Node. Bốn tổ hợp runtime thay thế đều chưa được đo~~ | ~~Chọn sai tầng MTProto thì hoặc nạp cả file nhiều GB vào RAM (hỏng ngay với phim thật), hoặc phải nuôi **hai** implementation MTProto song song~~ | 🟡 **Quyết định xong (2026-09-07)** — [SPIKE-10](./spikes/README.md#spike-10) đóng, chấp nhận rủi ro. Chọn `grammers-client` 0.10.0 cho công cụ ingest desktop, xem [ADR-0017](./adr/0017-grammers-cho-cong-cu-ingest-desktop.md). Trên tài khoản Telegram thật: M1/M2/M3/M5/M7/M8/P1 đều ĐẠT (RAM đỉnh 21 MB không tỉ lệ theo file; ngưỡng kích thước thật ~4000 MB Premium tìm được, lỗi `FILE_PARTS_INVALID`; catalog roundtrip byte-chính-xác; ranh giới crash FFmpeg xác nhận độc lập). R4 (`ferogram`) loại dứt khoát — không có API công khai an toàn để gắn `supportsStreaming`, xác nhận bằng `ConnectionReset` thật khi thử né. **Chấp nhận có chủ đích, chưa hoàn hảo**: M4 (throughput) cải thiện 13.5%→24.4% nhưng chưa qua ngưỡng lý tưởng 80%; M6 (`FLOOD_WAIT`) để ngỏ, không chủ động ép — đúng lý do đóng 🟡 thay vì 🟢. Vụ cược "một thư viện MTProto" của [ADR-0003](./adr/0003-chon-thu-vien-mtproto-gramjs.md) từ nay chỉ còn áp dụng cho `apps/web` |
