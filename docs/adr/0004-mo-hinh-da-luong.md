...
Đường "stream path" (SW ↔ main thread ↔ Core Worker, §"Đường đi của một byte" [ADR-0005](./0005-streaming-qua-service-worker-http-range.md)) đã chạy thật end-to-end, verify bằng phát video thật trên tài khoản thật (Windows + Chrome). Thiết kế bắc cầu cụ thể **không dùng port lâu dài**: mỗi request chunk/info, SW tự tạo một `MessageChannel` MỚI, gửi `port2` kèm request tới client qua `client.postMessage(msg, [port2])`, và chỉ chờ đúng MỘT lần trả lời trên `port1` — main thread chỉ cần đăng ký ĐÚNG MỘT listener `message` trên `navigator.serviceWorker` lúc bootstrap (`stream-bridge.ts`), không cần bookkeeping cổng nào ở phía nó. Đơn giản hơn phương án "SW chủ động tìm client qua `clients.matchAll()` rồi giữ port cố định" mà bản gốc ADR này phác thảo — vẫn giữ đúng bất biến "SW không mở kết nối MTProto", chỉ khác cách bắc cầu.

**Phát hiện thật (khiến playback không chạy được cho tới khi vá):** thiếu `self.clients.claim()` trong `activate` của SW. Không có dòng này, LẦN ĐẦU TIÊN một tab mở app, SW dù đã `activate` xong vẫn KHÔNG điều khiển tab đó cho tới khi user tự reload lần hai — mọi fetch `/_stream/*` đi thẳng ra mạng (404), player im lặng không phát được, không lỗi rõ ràng. `clients.claim()` chỉ ảnh hưởng "worker đã active có nhận điều khiển tab đang mở hay không", **không** ảnh hưởng thời điểm activate — an toàn với quyết định không `skipWaiting()` của [ADR-0012 §5](./0012-trien-khai-static-pwa-va-cau-truc-workspace.md) (không cắt ngang luồng đang phát khi có bản mới). Đã vá: `sw/sw.ts` gọi `event.waitUntil(self.clients.claim())` trong `activate`.

Cũng phát hiện: `<video>` không có `playsinline` sẽ tự nhảy sang trình phát fullscreen GỐC của hệ điều hành trên iOS Safari khi phát — che kín mọi UI web (kể cả overlay debug DOM). Đã thêm `playsinline`/`webkit-playsinline` cho `<video>` trong `player.html`.

Correlation id (bất biến đã ghi ở Quyết định gốc) dùng xuyên suốt: mỗi request chunk/info có `correlationId` riêng, cho phép log gộp 3 ngữ cảnh khi debug (xem `apps/web/src/app/debug/` — trang `#/debug` mà ADR này gọi là "hạng mục hạ tầng" chưa xây thành route riêng, nhưng đã có bản tối giản: log hiện NGAY TRONG trang play qua `?debug=1`, không phải overlay DOM rời — tránh đúng vấn đề fullscreen iOS nêu trên).
