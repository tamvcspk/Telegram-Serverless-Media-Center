...
## Cập nhật sau khi Accepted (2026-09-13, trả lời Đ2 + đóng gói DLL FFmpeg thật lần đầu)
...
[SPIKE-10 § Đ2](../spikes/README.md) để ngỏ câu hỏi "bộ FFmpeg phân phối được giữ ở LGPL hay kéo GPL (qua x264, cần cho re-encode Hạng D)?", đánh dấu "chưa đo" mọi nơi. **Trả lời bằng bằng chứng thật, không suy đoán** — lần đầu app này thật sự đóng gói `.exe` phân phối (`cargo tauri build`, xem addendum ADR-0017 tương ứng):

- `grep` chuỗi configure nhúng sẵn trong `avcodec-61.dll` (vcpkg `ffmpeg:x64-windows`) ra đúng `--disable-libx264 --disable-libx265` — build KHÔNG link x264/x265.
- Viết `ingest-ffmpeg/examples/check_h264_encoder.rs`, chạy `cargo run --example check_h264_encoder -p ingest-ffmpeg` — xác nhận encoder H264 THẬT được `ffmpeg-next` chọn ở `reencode.rs` (Hạng D) là **`h264_mf`** ("H264 via MediaFoundation") — codec của Windows, KHÔNG PHẢI `libx264` như comment gốc trong `reencode.rs` mô tả (comment đó copy nguyên từ ví dụ chính thức `examples/transcode-x264.rs` của `ffmpeg-next`, vốn giả định một build CÓ `libx264`).

**Kết luận Đ2: giữ được LGPL, KHÔNG kéo GPL.** `share/ffmpeg/copyright` của gói vcpkg xác nhận LGPL 2.1. Khớp đúng nhánh "Đ2 nói bộ FFmpeg phân phối được giữ ở LGPL" trong cây quyết định phụ của SPIKE-10 — người dùng chấp nhận nghĩa vụ LGPL (dynamic-link, không static — giữ khả năng đổi bản phân phối FFmpeg sau này mà không phải build lại từ một cấu hình đã lỡ static) cho một dự án cá nhân.

**Phát hiện phụ ngoài dự kiến (không phải giả định trước khi đo):** vì encoder thật là `h264_mf` chứ không phải `libx264`, dòng `x264_opts.set("preset", "medium")` ở `reencode.rs` gần như chắc chắn bị bỏ qua thầm lặng (AVOption của `libx264`, `h264_mf` không hiểu, `open_with()` không strict-check option lạ) — encode Hạng D vẫn chạy đúng và đã verify thật thành công trên tài khoản Telegram (2026-09-12), chỉ là bằng tham số MẶC ĐỊNH của `h264_mf`, không phải preset developer từng định chọn. Đã sửa comment trong `reencode.rs` cho đúng sự thật — **CHƯA sửa hành vi** (không đổi option/encoder mà không hỏi trước, pipeline đang chạy đúng trên tài khoản thật).
