...
## Cập nhật sau khi Accepted (2026-08-27, slice UI Login)

> Theo quy tắc ở [docs/adr/README.md](./README.md): không sửa nội dung Quyết định đã Accepted ở trên. Mục này chỉ ghi nhận hai phát hiện thật khi dựng lại màn Auth (F1.1) theo Màn hình 1 của [docs/ux-design.md](../ux-design.md) — quyết định gốc **vẫn đứng vững**.

**1. `<mat-icon>` mặc định phụ thuộc font ligature — dự án không có font đó.** `MatStepper` (và nhiều component Material khác) tự vẽ trạng thái "hoàn tất"/"sửa" bằng `<mat-icon>done</mat-icon>` / `<mat-icon>edit</mat-icon>` — đây là **tên ligature**, chỉ hiển thị đúng thành icon khi có font Material Symbols/Icons nạp sẵn. Bất biến #8 của dự án ("icon inline", không CDN font) có nghĩa font đó **không tồn tại** trong bundle. Hệ quả thật gặp: icon "done" render ra chữ "done" thô bị cắt cụt trong khung tròn nhỏ, trông như ký tự lỗi — không phải icon vỡ, mà là icon **không có font để hiển thị**. Sửa bằng cơ chế override chính thức của `MatStepper` (`<ng-template matStepperIcon="done">` chèn SVG inline), không cố nhúng font. **Rút ra: mọi component Material có icon mặc định (`MatStepper`, `MatChip` nút xoá, `MatSelect` mũi tên dùng CSS chứ không phải `mat-icon` nên an toàn…) phải kiểm tra xem trạng thái mặc định của nó có gọi `<mat-icon>` với ligature hay không TRƯỚC khi dùng, không phải sau khi thấy vỡ.**

**2. `MatStepper` `[linear]="true"` có race điều kiện thật với binding qua signal (không dùng Reactive Forms `stepControl`).** Khi điều khiển `selectedIndex` và `completed` bằng `[selectedIndex]="stepIndex()"` trên `<mat-stepper>` (cha) và `[completed]="stepIndex() > 0"` trên `<mat-step>` (con) — cả hai đọc từ cùng một signal — Angular áp input binding của **phần tử cha trước, phần tử con sau**, trong CÙNG một lượt change detection (thứ tự tài liệu/nesting, không phải thứ tự signal được `.set()`). `CdkStepper._anyControlsInvalidOrPending()` (linear mode, không có `stepControl`) đọc `step.completed` **tại thời điểm setter `selectedIndex` chạy** — tức đọc giá trị CŨ từ lượt render trước, vì `completed` của bước vừa xong CHƯA kịp ghi. Bước chuyển đầu tiên bị chặn ÂM THẦM (không lỗi, không log), đứng yên ở step cũ cho tới khi một tương tác khác (bất kỳ) kích hoạt thêm một lượt CD nữa để `completed` "kịp" ghi trước khi `selectedIndex` được thử lại. Tái hiện thật trên staging: submit xong step 1, UI không tự nhảy step 2, phải bấm gì đó thủ công mới "qua". **Sửa:** `[linear]="false"` (bỏ hẳn gate `_anyControlsInvalidOrPending`, chỉ áp dụng khi `linear=true`); giữ `[editable]="false"` từng step — cơ chế này độc lập với `linear`, vẫn chặn bấm NGƯỢC lại step đã qua. Đánh đổi chấp nhận: mất khả năng chặn bấm TỚI TRƯỚC một step chưa mở qua header — vô hại ở đây vì step chưa mở chưa có `pendingCode`/`pendingPassword`, submit sớm chỉ no-op. **Rút ra: đừng lái `MatStepper` linear bằng state bên ngoài qua binding hai chiều (`selectedIndex` + `completed` cùng derive từ một signal) — hoặc dùng `stepControl` thật (Reactive Forms, để `_markAsInteracted()` tự chạy đúng thứ tự trong cùng một lệnh gọi), hoặc tắt `linear` và tự lo phần chặn điều hướng.**
