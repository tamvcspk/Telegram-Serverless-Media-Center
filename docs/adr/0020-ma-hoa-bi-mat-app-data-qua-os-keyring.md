...
**Chưa verify:** luồng đầy đủ qua `cargo tauri dev` + tài khoản Telegram thật — đăng nhập lưu `credentials.json` qua keyring rồi mở lại app đọc đúng (không hỏi lại OTP); TMDB key qua màn Cài đặt (nhập/đổi/xoá) đọc/ghi đúng qua keyring. Checklist ở [docs/pending-device-tests.md](../pending-device-tests.md#gui-ingest-desktop-appstsmc-ingest-desktop--mã-hoá-app-data-qua-os-keyring-2026-09-14).
