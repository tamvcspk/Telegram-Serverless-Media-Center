...
>
> **Trạng thái so với code hiện tại:** phần lớn các màn hình dưới đây **chưa được xây** (xem [CLAUDE.md](../CLAUDE.md) để biết slice nào đã chạy thật). Auth (F1.1) đã có bản chạy thật trên staging, nhưng theo bản thiết kế **cũ** (Stepper ngang trong `MatCard` canh giữa, không phải vertical/full-bleed của bản mobile-first này) — xem [ADR-0016 addendum 2026-08-27](./adr/0016-angular-material-va-cdk.md#cập-nhật-sau-khi-accepted-2026-08-27-slice-ui-login). Bản vẽ Màn hình 1 dưới đây là đích **mới**, chưa implement. Browse (Dashboard rút gọn — chưa có bottom nav) và Player đã có bản chạy thật nhưng ở mức tối giản hơn mockup; Collections, Sources, Settings, Metadata Editor chưa có UI.
>
