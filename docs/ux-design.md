...
| Nhật ký **[Đã code + verify ĐẠT luồng chính, 2026-09-15]** | Log kỹ thuật, copy được | Chỗ để dán khi báo lỗi; không log session/token |
...
> **[Đã code + verify ĐẠT luồng chính, 2026-09-15]** Nhật ký (route `/logs`, icon riêng ở topbar Workspace cạnh "Trình quản lý catalog") — màn CUỐI CÙNG còn thiếu của phụ lục A.4, đóng gap `docs/roadmap.md` § Ingest "Nhật ký (A.4) — chưa bắt đầu". Hiển thị lại TOÀN BỘ nội dung file log kỹ thuật mà `tauri_plugin_log` đã ghi từ 2026-09-11 (bật cả ở release, chỉ ghi thao tác giao thức MTProto cấp thấp — connect/salt/request-response, KHÔNG có `auth_key`/session token, xem doc comment `src-tauri/src/lib.rs::run()`) — trước slice này chỉ xem được bằng cách tự mở file ở app-data, giờ có màn đọc lại + nút "Sao chép" (qua `@tauri-apps/plugin-clipboard-manager`/`tauri-plugin-clipboard-manager`, plugin Tauri chính thức — không dùng `navigator.clipboard` trần vì hành vi Clipboard API trong webview Tauri không đảm bảo nhất quán giữa `cargo tauri dev` và bản đóng gói) + "Làm mới" (đọc thủ công, không tự poll). Command mới `read_app_log()` (`src-tauri/src/logs.rs`) KHÔNG thuộc `IngestRpc` (không đụng MTProto — cùng nhóm `tmdb_search`/`probe_media`), tự tái tạo đúng đường dẫn file plugin dùng nội bộ (`app_log_dir()/<package_info().name>.log`, không có API công khai để hỏi lại plugin đường dẫn này). Rotation mặc định `KeepOne` của `tauri_plugin_log` (giữ tối đa 40KB, xoá hẳn bản cũ khi vượt) nên đọc trọn file không cần tự phân trang. `cargo build`/`ng build`/`npm run lint` sạch (`cargo clippy --all-targets` KHÔNG chạy được trong phiên code — tranh chấp file lock DLL FFmpeg với phiên `cargo tauri dev` đang chạy song song của user, không phải lỗi code). **Verify 2026-09-15, ĐẠT** cho luồng chính (hiện log đúng, nội dung không lộ `auth_key`/session token/OTP, "Sao chép" dán đúng) qua `cargo tauri dev` + tài khoản thật — "Làm mới", nhánh file log chưa tồn tại, rotation 40KB, và "Sao chép" ở bản đóng gói chưa xác nhận riêng, checklist ở [docs/pending-device-tests.md](./pending-device-tests.md#gui-ingest-desktop-appstsmc-ingest-desktop--nhật-ký-2026-09-15).
