...
> **[Đã code, 2026-09-15, CHƯA verify thiết bị thật]** Khối thứ NĂM **"Đồng bộ"** — KHÔNG thuộc bốn khối gốc của mockup ASCII ở trên (Tài khoản/Lưu trữ/Mạng/Chẩn đoán), đặt giữa "Mạng" và "Chẩn đoán". Đóng gap đã ghi sẵn ở [ADR-0009 § Hệ quả](./adr/0009-dong-bo-state-event-log-va-snapshot.md#hệ-quả) ("Kênh state có thể phình nếu compaction không chạy... → thêm chỉ báo tình trạng đồng bộ trong Cài đặt"): chỉ báo `compactionAgeLabel` (đọc `SyncMetaRecord.lastSnapshotAt` qua `liveQuery`, hiện "chưa từng nén" nếu chưa từng compact — không bịa số ngày) + `MatSlider` chỉnh ngưỡng nén-theo-tuổi (1-30 ngày, mặc định VẪN 7 ngày như ADR-0009 gốc, chỉ THÊM khả năng chỉnh, không đổi mặc định) ghi qua `client.setSetting('compactionMaxSnapshotAgeDays', days)`. Cùng lúc vá một bug thật phát hiện qua rà soát mã nguồn (`fetchEventsSince()` trước đây chỉ đọc đúng MỘT trang 500 message rồi coi như hết, có thể âm thầm bỏ sót event nếu compaction không chạy đủ lâu — nay tự lặp trang tới khi đọc hết). Chi tiết đầy đủ: [ADR-0009 § addendum 2026-09-15](./adr/0009-dong-bo-state-event-log-va-snapshot.md#cập-nhật-sau-khi-accepted-2026-09-15-rà-soát-brainstorm-2026-08-29--vá-bug-thật). `npm run test:libs`/`ng build`/`npm run lint` sạch — CHƯA verify bằng tài khoản thật (compaction chưa từng tự phát sinh trong một lần dùng bình thường).

## Phụ lục A: Công cụ ingest desktop (GUI Tauri)

> **Trạng thái (2026-09-12): Đăng nhập + Chọn kênh (hàng A.4 tương ứng) đã là code thật, chạy được; workspace ba vùng (A.3) đã có BẢNG METADATA + HÀNG ĐỢI + "Upload" cho CẢ 4 hạng A/B/C/D — Hạng C VÀ Hạng D đều đã verify thật (tài khoản thật)** (`apps/tsmc-ingest-desktop`, xem [ADR-0017](./adr/0017-grammers-cho-cong-cu-ingest-desktop.md) + [README](../apps/tsmc-ingest-desktop/README.md)) — thanh chi phí của A.3, cộng 2 màn A.4 còn lại (Trình quản lý catalog, Nhật ký), vẫn chỉ là bản vẽ. **Lệch mockup A.3 gốc bên dưới (theo yêu cầu user 2026-09-12):** hàng đợi (cột trái) và bảng metadata (cột phải) KHÔNG còn là hai cách nhìn của cùng một danh sách như mockup gốc mô tả — giờ tách biệt hẳn (bảng chỉ chứa file CHƯA upload, hàng đợi chỉ chứa file ĐÃ bấm Upload), xem ghi chú "Đã chạy thật một phần" ngay dưới mục A.3. Nguồn gốc: brainstorm 2026-09-05, sau khi [SPIKE-09](./spikes/README.md#spike-09) đóng 🟢 và user quyết định hướng GUI Tauri (quyết định này ADR-0013 để ngỏ từ 2026-08-29 — xem [ADR-0013](./adr/0013-bot-dong-hanh-va-pipeline-ingest.md)).
>
...
> **[Đã chạy thật một phần, 2026-09-12]** Vùng HÀNG ĐỢI (cột trái) — kéo thả file/folder, probe bằng `ingest-ffmpeg` (native `ffmpeg-next`, không cần cài ffmpeg lên máy), hiện hạng màu 🟢A/🟡B/🟠C/🔴D NGAY sau khi probe xong, đúng nguyên tắc UX A.2 mục 2 — **verify thật bằng file season thật** (xem [docs/changelog.md § 2026-09-12, verify thật](./changelog.md#2026-09-12--gui-ingest-desktop-workspace-ba-vùng--verify-thật-vùng-hàng-đợi-bằng-file-season-thật)). VÙNG BẢNG METADATA (cột phải) cũng đã code — chọn nhiều dòng (checkbox + "chọn tất cả"), gõ trực tiếp Title/Season/Ep/Năm từng ô, "Điền xuống" (Title/Season/Năm, từ dòng đầu tiên đã chọn xuống các dòng còn lại), "Đánh số tập tự động" (Season của dòng đầu + Episode tăng dần), "Xoá khỏi hàng" — seed/kế thừa ban đầu dùng lại nguyên `seedMetadataFromFilename()`/`inheritMetadata()` (`@tsmc/core-ingest`, cùng logic tuần tự CLI đã verify ở ADR-0013, khác CLI ở chỗ không hỏi xác nhận — kết quả kế thừa chỉ là gợi ý sẵn trong ô, admin tự sửa đè). Nút "Upload" (đặt trên `.metadata-toolbar`, appearance nổi khối) cũng đã code, nhận CẢ 4 hạng: remux/re-encode (Hạng D re-encode video thật, hỏi xác nhận riêng từng file kèm số phút ước tính qua dialog Material dùng chung — `shared/dialog/`, không còn `window.confirm()`, đúng nguyên tắc UX A.2 mục 1) → thumbnail → rút phụ đề (nhúng lẫn rời cạnh file) → upload video → upload phụ đề → publish catalog MỘT LẦN cho cả batch; `FLOOD_WAIT` đếm ngược đúng số giây rồi tự thử lại (không né bằng cách nào khác); nút Huỷ dừng đúng lần upload video đang chạy. **Verify thật (2026-09-12): upload Hạng C VÀ Hạng D đều thành công** qua tài khoản Telegram thật.
>
> **Redesign 2026-09-12 (lệch mockup ASCII ở trên, theo yêu cầu user):** "HÀNG ĐỢI" và "BẢNG METADATA" KHÔNG còn là hai cách nhìn của cùng một danh sách — bảng metadata (cột phải) chỉ chứa file CHƯA bấm Upload (nơi duy nhất sửa được), hàng đợi (cột trái) RỖNG cho tới khi bấm Upload rồi mới nhận file (kiểu "Queued files" của FileZilla / trạng thái gửi mail Apple Mail), mỗi dòng hàng đợi có `mat-progress-bar` mỏng dưới tên file + góc dưới sidebar hiện tổng tiến trình batch ("Đã tải X/N file — P%"). THANH CHI PHÍ (mockup dưới) vẫn chỉ là bản vẽ — chưa code. Chi tiết: [docs/changelog.md § 2026-09-12, verify Hạng D + tách hàng đợi](./changelog.md#2026-09-12--gui-ingest-desktop-verify-thật-hạng-d--tách-hàng-đợi-khỏi-bảng-metadata--shared-dialog).
...
| Cài đặt **[Đã chạy thật, 2026-09-14 — thêm mới, KHÔNG có trong mockup A.3/A.4 gốc]** | Tài khoản (hiển thị số điện thoại/API_ID/API_HASH che một phần, cộng nút Đăng xuất — [ADR-0017 § addendum](./adr/0017-grammers-cho-cong-cu-ingest-desktop.md#cập-nhật-sau-khi-accepted-2026-09-14-thêm-sign_out-vào-ingestrpc)) + TMDB (xem trạng thái, đổi key, xoá key) | Đăng xuất giữ nguyên `credentials.json`/`tmdb_api_key.json` (đăng nhập lại nhanh, chỉ cần OTP) — không có luồng "đổi hẳn tài khoản" riêng, tự sửa đè form. Bí mật app-data mã hoá qua OS keyring, không còn plaintext mặc định ([ADR-0020](./adr/0020-ma-hoa-bi-mat-app-data-qua-os-keyring.md)/[ADR-0021](./adr/0021-ma-hoa-session-sqlite-qua-session-tu-implement.md)) |
| Trình quản lý catalog **[Đã code, 2026-09-15, CHƯA verify thiết bị thật]** | Bảng toàn bộ item đang có, đối soát với message thật trong kênh, sửa/xoá/re-publish | Mọi dữ liệu đọc từ Telegram là **không tin cậy** — validate schema + kẹp độ dài như Màn hình 6 ([ADR-0011 §3](./adr/0011-bao-mat-session-va-noi-dung-khong-tin-cay.md)) |
| Nhật ký **[Đã code, verify ĐẠT một phần + vá 2 bug thật, 2026-09-15]** | Log kỹ thuật, copy được | Chỗ để dán khi báo lỗi; không log session/token |

> **[Đã code, 2026-09-15, CHƯA verify thiết bị thật]** Trình quản lý catalog (route `/catalog`, icon riêng ở topbar Workspace cạnh ⚙) — bảng CDK Virtual Scroll toàn bộ item catalog.json đang ghim, sửa Title/Năm/Season/Ep trực tiếp. **Đối soát chỉ MỘT CHIỀU** (chốt phạm vi có chủ đích, không làm rộng hơn ở slice này): nút "Đối soát với kênh" kiểm tra catalog item nào trỏ tới message đã bị xoá trên kênh (`IngestRpc::check_deleted_messages()`, tra đúng tập msgId catalog có qua `get_messages_by_id()` — không quét lịch sử kênh, chính xác tuyệt đối, tự nhiên bounded). CHƯA phát hiện chiều ngược lại (file mồ côi có trong kênh nhưng thiếu trong catalog) — để dành slice sau. Menu mỗi dòng: "Xoá khỏi catalog" (chỉ gỡ entry, staging tới khi bấm "Lưu catalog") hoặc "Xoá khỏi catalog + xoá message trên kênh" (`IngestRpc::delete_message()`, cảnh báo riêng tone warn — hành động mạng không hoàn tác được, chạy ngay lúc xác nhận). Chi tiết: [ADR-0017 § addendum 2026-09-15](./adr/0017-grammers-cho-cong-cu-ingest-desktop.md#cập-nhật-sau-khi-accepted-2026-09-15-trình-quản-lý-catalog-ingestrpc-thêm-2-thao-tác).

> **[Đã code, 2026-09-15]** Nhật ký (route `/logs`, icon riêng ở topbar Workspace cạnh "Trình quản lý catalog") — màn CUỐI CÙNG còn thiếu của phụ lục A.4, đóng gap `docs/roadmap.md` § Ingest "Nhật ký (A.4) — chưa bắt đầu". Hiển thị lại TOÀN BỘ nội dung file log kỹ thuật mà `tauri_plugin_log` đã ghi từ 2026-09-11 (bật cả ở release, KHÔNG có `auth_key`/session token, xem doc comment `src-tauri/src/lib.rs::run()`) — trước slice này chỉ xem được bằng cách tự mở file ở app-data, giờ có màn đọc lại + nút "Sao chép" (qua `@tauri-apps/plugin-clipboard-manager`/`tauri-plugin-clipboard-manager`, plugin Tauri chính thức — không dùng `navigator.clipboard` trần vì hành vi Clipboard API trong webview Tauri không đảm bảo nhất quán giữa `cargo tauri dev` và bản đóng gói) + "Làm mới" (đọc thủ công, không tự poll). Command mới `read_app_log()` (`src-tauri/src/logs.rs`) KHÔNG thuộc `IngestRpc` (không đụng MTProto — cùng nhóm `tmdb_search`/`probe_media`), tự tái tạo đúng đường dẫn file plugin dùng nội bộ (`app_log_dir()/<package_info().name>.log`, không có API công khai để hỏi lại plugin đường dẫn này). Rotation mặc định `KeepOne` của `tauri_plugin_log` (giữ tối đa 40KB, xoá hẳn bản cũ khi vượt) nên đọc trọn file không cần tự phân trang. **Verify 2026-09-15, ĐẠT** cho luồng chính (hiện log đúng, nội dung không lộ `auth_key`/session token/OTP, "Sao chép" dán đúng) qua `cargo tauri dev` + tài khoản thật.
>
> **Cập nhật tiếp trong ngày — 2 bug thật phát hiện lúc verify, đã vá, CHƯA verify lại:** (1) log KHÔNG tự cuộn xuống cuối — `afterNextRender()` (Angular) đăng ký một lần ở constructor chạy trước khi `<pre>` tồn tại trong DOM (chỉ render sau khi hết `loading()`), cùng lớp bug đã ghi ở `apps/web/src/app/browse/browse.ts::gridMeasure`; vá bằng gọi `afterNextRender()` MỚI mỗi lần `load()`. (2) "Làm mới" không thấy nội dung mới — KHÔNG phải lỗi đọc file, mà do mức lọc `log::LevelFilter::Info` chặn hết log `debug!`/`trace!` mà `grammers-mtsender`/`grammers-mtproto` phát ra cho MỌI RPC bình thường sau lần connect/sinh `auth_key` đầu phiên (chỉ event hiếm mới ở mức `Info`); nâng lên `Debug` (`lib.rs`) SAU KHI đối chiếu trực tiếp mã nguồn `grammers-mtsender`/`grammers-mtproto`/`grammers-client` 0.10.0 xác nhận các dòng `debug!`/`trace!` chỉ in `msg_id`/tên kiểu TL/số byte/mã lỗi RPC, không dòng nào in giá trị trường request (số điện thoại, OTP, mật khẩu 2FA) hay `auth_key` — giữ đúng cam kết "không log session/token". `cargo build`/`cargo clippy --workspace -- -D warnings`/`ng build`/`npm run lint` sạch — CHƯA verify lại bằng tài khoản thật, checklist ở [docs/pending-device-tests.md](./pending-device-tests.md#gui-ingest-desktop-appstsmc-ingest-desktop--nhật-ký-2026-09-15) (gồm bước mới: đối chiếu log THẬT ở mức `Debug` không lộ trường nhạy cảm — audit mã nguồn không thay được một lượt quan sát thật).
