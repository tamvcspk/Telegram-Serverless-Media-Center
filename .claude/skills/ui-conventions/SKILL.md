...
| **Sub-page** | Màn 6 (Metadata Editor, chưa route) / Màn 7 (Settings, **đã implement 2026-08-28**) | Header `<` quay lại, có thể có sticky bottom bar (Metadata Editor) | Settings: route đơn `settings`, `canActivate: [authGuard]` (cần session để hiển thị tài khoản + syncEngine đã init() cho forceFlush()/setMaxConcurrency()), KHÔNG dưới `home` (không thuộc Bottom Nav — vào qua icon ⚙️ ở Browse). Metadata Editor: chưa route, chưa có UI thật. Khi có ≥2 sub-page giống hệt nhau mới tách cha layout header dùng chung (rule-of-three như `app/shared/` ở mục 1) — hiện chỉ có một, chưa tới ngưỡng đó |
